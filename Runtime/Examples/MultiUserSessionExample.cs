    ...
    public class MultiUserSessionExample
    {
        public static void RunExample()
        {
            ...
            foreach (var user in users)
            {
                ...
                Debug.Log($"用户 {user} 创建会话: {session.Name}");
            }
            ...
            foreach (var user in users)
            {
                ...
                Debug.Log($"用户 {user} 在会话中创建数据集: {dataset.Name}");

                // 每个用户打开全局数据集的副本
                var globalData = store.CreateTabular("GlobalData");
                var copyName = $"{user}_Copy";
                var copy = session.OpenDataset("GlobalData", copyName);
                Debug.Log($"用户 {user} 打开数据集副本: {copy.Name}");
            }

            // 显示会话统计
            var stats = store.SessionManager.GetStatistics();
            Debug.Log($"会话统计: {stats.TotalSessions} 个会话, 总共 {stats.TotalDatasets} 个数据集, 平均每会话 {stats.AverageDatasetsPerSession:F1} 个数据集");

            // 模拟用户完成工作并关闭会话
            foreach (var user in users)
            {
                var session = userSessions[user];
                Debug.Log($"用户 {user} 完成工作，关闭会话: {session.Name}");
                store.SessionManager.CloseSession(session.Id);
            }

            // 检查剩余会话
            stats = store.SessionManager.GetStatistics();
            Debug.Log($"关闭用户会话后统计: {stats.TotalSessions} 个会话, 总共 {stats.TotalDatasets} 个数据集");

            // 清理
            store.Dispose();
            Debug.Log("完成多用户会话管理示例");
        }
    }
}