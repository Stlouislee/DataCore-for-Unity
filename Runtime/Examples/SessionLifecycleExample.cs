...
namespace AroAro.DataCore.Examples
{
    ...
    public class SessionLifecycleExample
    {
        public static void RunExample()
        {
            ...
            Debug.Log($"创建会话: {session1.Name}, {session2.Name}");
            ...
            // 3. 检查会话状态
            Debug.Log($"会话统计: {store.SessionManager.GetStatistics().TotalSessions} 个会话");

            // 4. 关闭特定会话
            Debug.Log($"关闭会话: {session1.Name}");
            store.SessionManager.CloseSession(session1.Id);

            // 5. 检查剩余会话
            Debug.Log($"剩余会话数: {store.SessionManager.GetStatistics().TotalSessions}");

            // 6. 清理空闲会话（模拟）
            var idleTimeout = TimeSpan.FromMinutes(1);
            Debug.Log($"清理空闲超过 {idleTimeout.TotalMinutes} 分钟的会话");
            var cleanedCount = store.SessionManager.CleanupIdleSessions(idleTimeout);
            Debug.Log($"清理了 {cleanedCount} 个空闲会话");
            ...
            // 8. 最终检查
            Debug.Log($"最终会话数: {store.SessionManager.GetStatistics().TotalSessions}");

            Debug.Log("完成会话生命周期管理示例");
        }

        private static void SimulateSessionActivity(ISession session, string user)
        {
            Debug.Log($"用户 {user} 在会话 {session.Name} 中工作...");
            ...
            Debug.Log($"用户 {user} 完成会话 {session.Name} 中的工作");
        }
    }
}